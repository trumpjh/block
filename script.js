...
function applyItemEffect(item) {
    switch(item.effect) {
        ...
        case 'multiBall':
            if (balls.length < 5) {
                const originalBall = balls[0] || { x: paddle.x, y: paddle.y, dx: 4, dy: -4, radius: 8, trail: [] };
...
function checkCollision(obj, rect) {
    const objRadius = obj.radius || 15;
    return obj.x + objRadius > rect.x && obj.x - objRadius < rect.x + rect.width &&
           obj.y + objRadius > rect.y && obj.y - objRadius < rect.y + rect.height;
}
...
function update() {
    ...
    if (balls.length === 0 && gameState.gameStarted && !gameState.gameOver) {
        gameState.lives--;
